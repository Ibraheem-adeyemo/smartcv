...
import { ATMInService, Paginate, StatsA, StatsProps, TransactionBreakdownType } from "../../models"
...
export const TextChart: FC<TransactionBreakdownProps> = ({ showDetails = false, ...props }: TransactionBreakdownProps) => {
  ...
  return (
    <AppCard topic={<Text variant="card-header" size="card-header">Transaction Breakdown</Text>} >
      ...
      {!props?.loading.isLoading && props.stats ?
        <Flex flexWrap={"wrap"} gap="15px"> {props?.stats.map((x, i) => (
