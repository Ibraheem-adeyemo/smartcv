      ...
      <div style={{ width: '100%', backgroundColor: 'white', borderRadius:10, padding: 10 }}>
        {/* <ResponsiveContainer width="100%" height={300}> */}
          ...
          >
            ...
            <Line type="monotone" name='Approved' hide={lineShow.successCount} dataKey="Approved" strokeOpacity={opacity.pv} stroke="#8884d8" activeDot={{ r: 8 }} />
            <Line type="monotone" name='USers failure'dataKey="failedCount" hide={lineShow.failedCount} strokeOpacity={opacity.uv} stroke="#82ca9d" />
            {/* <Line type="monotone" dataKey="failedCount" hide={lineShow.failedCount} strokeOpacity={opacity.uv} stroke="red" /> */}
            <text x="220" y="40" dominantBaseline="hanging" fontSize="36" fontWeight="bold">{chartProps?.realTime?.interchangeName}</text>
          </LineChart>
        {/* </ResponsiveContainer> */}
...
export const IssuingLineChart = (props:IssuingLineChartProps) => {
    ...
    //   <ResponsiveContainer width='100%' height='100%'>
        <LineChart width={width} height={height} data={data}>
          ...
          <XAxis dataKey={dataKey} padding={{ left: 30, right: 30 }} tickLine={false} />
          <YAxis type='number' domain={[0, 1000]} tickLine={false} unit='m'  />
          <Tooltip />
          <Legend />
          {
            lines.map((line, i) => {
                return <Line key={i} type={line.type} dataKey={line.dataKey} stroke={line.stroke} strokeWidth={2} {...line?.activeDot} />
            }) 
          }
        </LineChart>
    //   </ResponsiveContainer>
...
const CustomXaxisText = (props:any) => {
    ...
    console.log(props)
    return(
    <Text color='red'>payload.value</Text>
    );
}
...
export const IssuingBarChart = (props: IssuingBarChartProps) => {
    ...
    return (
        <ResponsiveContainer width="80%" height="100%">
          ...
          <YAxis type='number' domain={[0, 1000]} tickLine={false} unit='m' axisLine={false} />
...
export const IssuingBarChartHorizontal = (props: IssuingBarChartProps) => {
    ...
    return (
        <ResponsiveContainer width="100%" height="80%">
          <BarChart data={data} layout="vertical" barGap={2}  margin={{
              top: 5,
              right: 30,
              left: 35,
              bottom: 5,
          ...
          <YAxis dataKey={'name'} type={'category'} name='Hello' tickLine={false} axisLine={false} />
