...
import {
  ...
  channelsMonitoringIcon,
  InterchangeDisconnectionIcon,
  ForwardQueue,
  AuthenticatedPage,
  menuNames,
  cookieKeys,
...
const AuthenticatedLayout: FC<AuthenticatedLayoutProps> = (
  ...
  const { userDetail, user, authMode, signOut, error, refreshAccessToken } =
  ...
  const isUserLoading =
    typeof userDetail === "undefined" /*|| typeof user === "undefined"*/ &&
    typeof error === "undefined";
  const isUserLoaded =
    typeof userDetail !== "undefined" /*&& typeof user !== "undefined"*/ &&
    (typeof error !== "undefined" || authMode !== "passport");
  const handleOnActive = () => {
    if (typeof window !== "undefined") {
      const timeLeft =
        new Date().getTime() -
        +getCookie(cookieKeys.tokenDurationDate) * 1000 * 60 * 60;
        ...
        {menuList.map(
          (x, i) =>
            x.link && (
              <MotionText
                color="brand.muted"
                animate={animationCount ? "show" : "hide"}
                initial={!animationCount ? "show" : "hide"}
                sx={{
              ...
              >
                <AppLink
                  ...
                  onClick={(e: any) => {
                    ...
                    router.push(
                      e.target.href ? e.target.href : e.currentTarget.href
                    );
                  }}
                  role="group"
                  _active={{
  ...
  return (
    <Grid sx={appLayoutSX}>
            ...
            <MenuList d="flex" gap="1px" flexDir="column" px="18px" pt="22px">
              <MenuItem as={Flex} gap="11px" alignItems="center">
                ...
                <Flex flexDir="column" gap="1px">
                  {isUserLoaded && (
                    <>
                      <Text size="dropdown-text" variant="dropdown-text-header">
                        {" "}
                        Hello, {user?.firstName}
                      </Text>
              ...
              <MenuItem>
                {isUserLoading && (
                  <SkeletonLoader
                    ...
                    height="15px"
                    columns={1}
                    loaderKey="profile-and-preferences"
                  />
                )}

                {isUserLoaded && (
                  <Text size="dropdown-tes">Profile and Preferences</Text>
                )}
              </MenuItem>
              <MenuDivider />
              <MenuItem onClick={() => signOut()}>
                {
                  /*!user &&*/ typeof error === "undefined" && (
                    <SkeletonLoader
                      rows={1}
                      width="100px"
                      height="15px"
      ...
      <GridItem sx={sidebarSX}>
        {isUserLoading && (
          <SkeletonLoader
            rows={5}
            width={["70px", "70px", "100px", "200px", "300px", "300px"]}
            columns={1}
            loaderKey="loader-list"
          />
        )}
        {isUserLoaded && <MenuLists />}
      </GridItem>
      {isUserLoading && (
        <GridItem d="flex" w="100%" alignItems="center" px="50px">
          <SkeletonLoader
            rows={1}
            width="200px"
            height="20px"
            columns={1}
            loaderKey="page-header"
          />
        </GridItem>
      )}
      {isUserLoaded && (
        <GridItem d="flex" w="100%" alignItems="center">
          {" "}
      ...
      {isUserLoading && (
        <Flex sx={skeletonLoaderForMainSX}>
          {" "}
          <SkeletonLoader
            rows={1}
            height="100%"
