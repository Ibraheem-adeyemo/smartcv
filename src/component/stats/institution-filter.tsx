...
const InstitutionFilter: FC = () => {
  ...
  const isTenantAndInstitutionLoading =
    typeof tenants === "undefined" && typeof institutionsError === "undefined";
  useEffect(() => {
    // debugger
    ...
    <>
      {typeof tenants !== "undefined" && (
        <SearchFilter
              ...
              ? [
                  {
                    label: "All",
                    value: "0",
                    selected: selectedTenantCode === "0",
                  },
                  ...map(tenants, (x, i) => ({
                    label: x.name,
                    value: x.code,
                    selected: x.code === selectedTenantCode,
                  })),
                ]
              : [
                  ...map(tenants, (x, i) => ({
          ...
          label="Institution"
          onSelected={(e) => changeSelectedTenantCode(e.value)}
          selected
        />
      )}
      {isTenantAndInstitutionLoading && (
        <SkeletonLoader
          rows={1}
          columns={1}
          width="100px"
          height="20px"
          loaderKey="tenant-and-institution"
        />
      )}
    </>
  );
